...
#' vis <- polmineR.three:::text(vis, .data=data.frame(coords, row.names=rownames(mtcars)), color="0xff111111")
#' vis
#' }
three <- function(type="base", bgColor="0x000000"){
  htmlTemplate <- scan(
    file=system.file("templates", paste(type, ".html", sep=""), package="polmineR.three"),
    what="character", sep="\n", quiet=TRUE
  )
  lookUp <- c("three.min.js", "TrackballControls.js", "AnaglyphEffect.js", "StereoEffect.js")
  jsPath <- sapply(lookUp, function(x) system.file("js", x, package="polmineR.three"))
